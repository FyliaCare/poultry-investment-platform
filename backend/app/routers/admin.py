...
def create_farm(payload: FarmIn, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def list_farms(db: Session = Depends(get_db), admin=Depends(require_admin)):
    return db.query(Farm).all()

@router.post("/batches", response_model=BatchOut)
def create_batch(payload: BatchIn, db: Session = Depends(get_db), admin=Depends(require_admin)):
    try:
...
def list_batches(db: Session = Depends(get_db), admin=Depends(require_admin)):
    return db.query(Batch).all()

@router.post("/batches/{batch_id}/activate")
def activate_batch(batch_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
def harvest_batch(batch_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
    b = db.get(Batch, batch_id)
    if not b:
        raise HTTPException(status_code=404, detail="Batch not found")
    b.status = "HARVESTED"
...
def simulate(batch_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
    b = db.get(Batch, batch_id)
    if not b:
        raise HTTPException(status_code=404, detail="Batch not found")
    total = simulate_payout_for_batch(db, b)
    return {"batch_id": batch_id, "simulated_total": total}

@router.post("/payouts/{batch_id}/execute")
def execute(batch_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
    b = db.get(Batch, batch_id)
    if not b:
        raise HTTPException(status_code=404, detail="Batch not found")
    payouts = execute_payout_for_batch(db, b)
    return {"count": len(payouts)}
